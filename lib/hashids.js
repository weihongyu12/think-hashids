...
class ThinkHashids {
  /**
   * @param {object} option
   * @param {string} option.salt
   * @param {number} option.minLength
   * @param {string} option.alphabet
   */
  constructor(option) {
    if (helper.isEmpty(option)) {
      option = defaultConfig;
    }
    this.hashids = new Hashids(option.salt, option.minLength, option.alphabet);
  }
